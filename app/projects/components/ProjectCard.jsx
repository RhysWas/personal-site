...
export default function ProjectCard({ data }) {
  const { title, year, desc, link, skills } = data;
      ...
      <div className="flex justify-between">
        ...
        {link ? (
          <div className="flex items-end">
            <Link href={link}>
              <p className="text-sm hover:text-cyan-600 hover:underline">
                Code
              </p>
            </Link>
          </div>
      ...
      {details ? (
        <div className="flex-1 overflow-auto flex-col h-full pb-2">
          ...
          <div className="flex justify-between mt-auto mb-2">
            ...
            <button
              className="place-self-end px-2 bg-red-300 rounded-full shadow-sm border-solid border-2 border-red-800"
        ...
        <div className="text-center py-8 h-full">
          ...
          <div className="flex justify-center z-10 whitespace-normal hover:flex hover:overflow-visible line-clamp-1 hover:line-clamp-none hover:font-bold pb-3">
            {skills.map((skill) => (
              <p key={skill} className="text-xs whitespace-nowrap px-1">
                {skill}
              </p>
            ))}
          </div>
          <button
            className="mt-auto mb-3 px-2 bg-cyan-200 border-solid border-orange-400 border-2 shadow-sm rounded-full text-sm"
